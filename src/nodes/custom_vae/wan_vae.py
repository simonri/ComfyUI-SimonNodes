...
class Encoder3d(nn.Module):
  def __init__(
    self, in_channels=3, dim=128, z_dim=4, dim_mult=[1, 2, 4, 4], num_res_blocks=2, attn_scales=[], temperal_downsample=[True, True, False], dropout=0.0
  ):
    super().__init__()
...
class Decoder3d(nn.Module):
  def __init__(
    self, out_channels=3, dim=128, z_dim=4, dim_mult=[1, 2, 4, 4], num_res_blocks=2, attn_scales=[], temperal_upsample=[False, True, True], dropout=0.0
  ):
    super().__init__()
    ...
    # output blocks
    self.head = nn.Sequential(RMS_norm(out_dim, images=False), nn.SiLU(), CausalConv3d(out_dim, out_channels, 3, padding=1))
...
class WanVAE(nn.Module):
  def __init__(
    ...
    # modules
    self.encoder = Encoder3d(in_channels, dim, z_dim * 2, dim_mult, num_res_blocks, attn_scales, self.temperal_downsample, dropout)
    self.conv1 = CausalConv3d(z_dim * 2, z_dim * 2, 1)
    self.conv2 = CausalConv3d(z_dim, z_dim, 1)
    self.decoder = Decoder3d(out_channels, dim, z_dim, dim_mult, num_res_blocks, attn_scales, self.temperal_upsample, dropout)
